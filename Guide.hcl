...
/*
To write your first provisioner, add the following block into your Packer template, inside the build block and underneath the sources assignment.


This block defines a shell provisioner which sets an environment variable named FOO in the shell execution environment and runs the commands in the inline attribute. This provisioner will wait 30 seconds before installing Redis, then create a file named example.txt that contains FOO is hello world.
